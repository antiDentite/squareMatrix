...
unsigned int dim=M_1.return_to_derived_2().dmn;
std::vector< std::vector<T> > M_2(dim);
...
unsigned int dim=return_to_derived_2().dmn;
...
template <class T,unsigned int dim> FULL<T,dim>::FULL():dmn(dim){
...
std::cerr<<"Error: invalid index values in call to operator(). ";
exit(1);
...
std::cerr<<"Error: invalid index values in call to entry(). ";
exit(1);
...
template<class T, unsigned int dim> FULL<T,dim>::decompBin::decompBin(const FULL<T,dim> & M_1){
...
if((U[i][0]>=0)*U[i][0] -(U[i][0]<0)*U[i][0]<THRESH<T>::value){valid_2=1;break;};
L[j][i]=s_1/U[i][0];};
...
if(valid_2){valid=0;} else{valid=1;};

};

///BindDecomp solver

template<class T, unsigned int dim> vct<T,dim> FULL<T,dim>::decompBin::solve(const vct<T,dim> & b){
vct<T,dim> x;
std::vector<T> y_1(dim);
T s(0);
if(valid==0){

std::cerr<<"Error: calling member function 'solve' when valid=0.";
exit(1);
...
template<class T,unsigned int dim> TRIDAG<T,dim>::TRIDAG():M(dim),dmn(dim){
...
///BinDecomp constructor
template<class T, unsigned int dim> TRIDAG<T,dim>::decompBin::decompBin(const TRIDAG<T,dim> & M_1){
bool valid_2(0);
T t;
if(dim==2){

L.resize(2); L[0].insert(L[0].begin(),1);L[1].resize(2);L[1][1]=1;
U.resize(2); U[0].resize(2); U[0][0]=M_1.M[0][0];
U[0][1]=M_1.M[0][1];
t=U[0][0];
if(((t>=0)*t -(t<0)*t)<THRESH<T>::value){valid=0;}
else{
L[1][0]=M_1.M[1][0]/M_1.M[0][0];

t=M_1.M[1][1]-M_1.M[0][1]*M_1.M[1][0]/M_1.M[0][0];

if(((t>=0)*t -(t<0)*t)<THRESH<T>::value){valid=0;}
else{valid=1;U[1].insert(U[1].begin(),t);};
...
L.resize(dim); U.resize(dim);
U[0].assign(1,M_1.M[0][1]);
t=M_1.M[0][0];

if(((t>=0)*t-(t<0)*t)<THRESH<T>::value){valid=0;}else{
U[0].insert(U[0].begin(),M_1.M[0][0]);

U[1].assign(1,M_1.M[1][2]);
t=M_1.M[1][1]-(M_1.M[1][0]/U[0][0])*M_1.M[0][1];
if(((t>=0)*t-(t<0)*t)<THRESH<T>::value){valid=0;} else{
U[1].insert(U[1].begin(),t);

for(int i=2;i<dim-1;i++){

U[i].assign(1,M_1.M[i][2]);
t=M_1.M[i][1]-(M_1.M[i][0]/U[i-1][0])*M_1.M[i-1][2];
if(((t>=0)*t-(t<0)*t)<THRESH<T>::value){valid_2=1;break;};
U[i].insert(U[i].begin(),t);};

if(valid_2){valid=0;} else{
t=M_1.M[dim-1][1]-(M_1.M[dim-1][0]/U[dim-2][0])*M_1.M[dim-2][2];
if(((t>=0)*t-(t<0)*t)<THRESH<T>::value){valid_2=1;}
else{
U[dim-1].insert(U[dim-1].begin(),t);
...
t=U[i-1][0];
if(((t>=0)*t-(t<0)*t)<THRESH<T>::value){valid=0;};
L[i].insert(L[i].begin(),M_1.M[i][0]/U[i-1][0]);
...
template<class T, unsigned int dim> vct<T,dim> TRIDAG<T,dim>::decompBin::solve(const vct<T,dim> & b){
...
template <class T,unsigned int dim> BAND<T,dim>::BAND(const unsigned int & p_1,const unsigned int & q_1):dmn(dim){
...
template<class T, unsigned int dim> BAND<T,dim>::decompBin::decompBin(const BAND<T,dim> & M_1){
bool valid_2(0); T t_2;
L.resize(dim); U.resize(dim);
for(int i=0;i<dim;i++){
if(M_1.p<=i){L[i].resize(M_1.p+1);L[i][M_1.p]=1;} else{L[i].resize(i+1);L[i][i]=1;};
if(M_1.q<=(dim-(i+1))){U[i].resize(M_1.q+1);} else{U[i].resize(dim-i);};
};
...
if(((t_2>=0)*t_2-(t_2<0)*t_2)<THRESH<T>::value){valid=0;} else{
for(int k=1;k<=std::min<int>(dim-1,M_1.p);k++){
L[k][0]=M_1.M[k][0]/U[0][0];
};
int adj;
...
for(int j=i+1;j<=std::min<int>(dim-1,i+M_1.p);j++){
adj=std::max<int>(0,j-M_1.p);
t=M_1.M[j][i-adj];
for(int k=0;k<i-adj;k++)t-=L[j][k]*U[k][i-k];
t_2=U[i][0];
if(((t_2>=0)*t_2-(t_2<0)*t_2)<THRESH<T>::value){valid_2=1;break;};
L[j][i-adj]=t/U[i][0];
...
template<class T, unsigned int dim> vct<T,dim> BAND<T,dim>::decompBin::solve(const vct<T,dim> & b){
int p=L[dim-1].size()-1;
...
///declarations for the threshold value for the
///LU decompositions:
