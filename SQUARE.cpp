...
template <class T, unsigned int dim> T & vct<T,dim>::operator()(const unsigned int & pos)
{
    return (*this)[pos];
};
...
template <class T,unsigned int dim> vct<T,dim> vct<T,dim>:: operator+(const vct<T,dim> & v_1)const
{
    vct v_2(*(this));
    v_2+=v_1;
    return v_2;
...
template<class T, unsigned int dim> FULL<T,dim>::decompBin::decompBin(const FULL<T,dim> & M_1)
{
    ...
    for(int i=0; i<dim; i++)
    {
        ...
        for(int j=i+1; j<dim; j++)
        {
            ...
            if((U[i][0]>=0)*U[i][0] -(U[i][0]<0)*U[i][0]<THRESH<T>::value)
            {
                valid_2=1;
                break;
            };
...
///build the first column of L
    ...
    if(((t_2>=0)*t_2-(t_2<0)*t_2)<THRESH<T>::value)
    {
        valid=0;
    }
    else
