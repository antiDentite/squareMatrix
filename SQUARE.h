...
for(int j=0;j<dim-1;j++)os<<M_1.entry(dim-1,j)<<',';
os<<M_1.entry(dim-1,dim-1);
...
///A template class for the threshold value for
...
FULL<T,dim> M_2; M_2.M=M_1.negation();
...
std::vector< std::vector<T> > L; ///for the lower triangular matrix
std::vector< std::vector<T> > U; ///for the upper triangular matrix
bool valid; ///to indicate whether we have a valid LU decomposition
decompBin(const FULL<T,dim> &);
vct<T,dim> solve(const vct<T,dim> &); ///Linear system solver
};
...
BAND <T,dim> M_2(M_1.p,M_1.q); M_2.M=M_1.negation();
...
///Container struct for the LU Decomposition
...
TRIDAG <T,dim> M_2; M_2.M=M_1.negation();
